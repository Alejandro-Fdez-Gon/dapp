...
contract('Uso normal de Asignatura:', accounts => {
    ...
    it("El nombre y curso deben ser BCDA y 2024", async () => {
        ...
        assert.equal(curso, "2024", "El curso debe ser 2023.");
    });
    ...
    it("Añadir los profesores.", async () => {
        const asignatura = await Asignatura.deployed();
    ...
    it("Solo el owner puede añadir profesores.", async () => {
        const asignatura = await Asignatura.deployed();
    ...
    it("Solo se puede evaluar a un alumno.", async () => {
        const asignatura = await Asignatura.deployed();

        try {
            await asignatura.califica(desconocido, 2, 2, 800, { from: profesores[0].addr });
            assert.fail("Solo se puede evaluar a un alumno.");
        } catch {
        }
    });

    it("Solo se puede calificar una evaluación existente.", async () => {
        const asignatura = await Asignatura.deployed();

        try {
            await asignatura.califica(alumnos[0].addr, evaluaciones.length, 2, 800, { from: profesores[0].addr });
            assert.fail("Solo se puede calificar una evaluación existente.");
        } catch {
        }
    });
});
